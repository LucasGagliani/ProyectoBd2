...
export default function DriverDashboard() {
  ...
  const loadData = async () => {
    try {
      const [driverRes, tripsRes] = await Promise.all([getDriverMe(), getMyTrips()])
  ...
  const setLocation = async (barrio) => {
    ...
    try {
      ...
      setLocationMsg(`Ubicación actualizada: ${barrio}`)
      setTimeout(() => setLocationMsg(''), 3000)
    } catch {
      setLocationMsg('Error al actualizar ubicación')
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10 space-y-6">
      ...
      <div className="bg-white border rounded-xl p-5">
        ...
        <div className="flex gap-2 flex-wrap">
          {ESTADOS.filter(e => e !== driver.estado).map(e => (
            <button key={e} onClick={() => changeStatus(e)} disabled={loading}
      ...
      {/* Ubicación */}
      <div className="bg-white border rounded-xl p-5">
        <h2 className="font-semibold mb-1">Tu ubicación</h2>
        <p className="text-xs text-gray-500 mb-3">Necesitás tener una ubicación activa para recibir viajes</p>
        {driver.latitud_actual ? (
          <p className="text-sm text-green-600 mb-3">
            ✓ Posición activa: {selectedBarrio || `${driver.latitud_actual}, ${driver.longitud_actual}`}
          </p>
        ) : (
          <p className="text-sm text-red-500 mb-3">⚠ Sin ubicación — no recibirás viajes asignados</p>
        )}
        <div className="flex flex-wrap gap-2">
          {Object.keys(BARRIOS).map(b => (
            <button key={b} onClick={() => setLocation(b)}
              className={`px-3 py-1 rounded-full text-sm border transition ${selectedBarrio === b || (driver.latitud_actual && !selectedBarrio) ? '' : ''} border-gray-300 hover:border-black hover:bg-gray-50`}>
              {b}
            </button>
          ))}
        </div>
        ...
        {trips.length === 0 ? (
          ...
          <div className="space-y-3">
            {trips.map(trip => {
              ...
              return (
                <button key={trip.id_viaje} onClick={() => navigate(`/trips/${trip.id_viaje}`)}
                  ...
                  <p className="text-xs text-blue-600 mt-2 font-medium">Tocá para ver y gestionar →</p>
                </button>
              )
            })}
          </div>
      ...
      {/* Info */}
      <div className="bg-white border rounded-xl p-5">
        <h2 className="font-semibold mb-3 text-gray-700">Información</h2>
        <div className="space-y-2 text-sm text-gray-600">
          <Row label="Licencia" value={driver.nro_licencia} />
          <Row label="Calificación" value={`⭐ ${driver.calificacion_promedio}`} />
